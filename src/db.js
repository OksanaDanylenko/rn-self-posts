...
export class DB {
    ...
    return new Promise((resolve, reject) => {
      db.transaction((tx) =>
        tx.executeSql(
          ...
          [],
          resolve,
          (_, error) => reject(error), // _ - SQL request
        ),
      );
    });
  }
  ...
  static createPost({ text, date, booked, img }) {
    return new Promise((resolve, reject) => {
      db.transaction((tx) => {
        tx.executeSql(
          `INSERT INTO posts (text, date, booked, img) VALUES (?, ?, ?, ?)`, //? - for security reasons
          [text, date, 0, img],
          (_, result) => resolve(result.insertId), //id that gives DB for this element
          (_, error) => reject(error),
        );
      });
    });
